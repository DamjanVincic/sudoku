...
TEST_F(SudokuTest, CheckSolutionTest) {
    ...
    ASSERT_TRUE(sudoku.checkSolution());
    ASSERT_EQ(sudoku.getNumberOfInvalidCells(), 3);

    sudoku.solveSudoku();
    ASSERT_EQ(sudoku.getNumberOfValidCells(), 48);

    // Board that is not the same as the original one
    std::vector<std::vector<unsigned short>> wrongBoard = {
            {4, 3, 4, 6, 7, 8, 9, 1, 2},
            {6, 7, 2, 1, 9, 5, 3, 4, 8},
            {1, 9, 8, 3, 4, 2, 5, 6, 7},
            {8, 5, 9, 7, 6, 1, 4, 2, 3},
    ...
    sudoku.setSolution(wrongBoard);
    ASSERT_FALSE(sudoku.checkSolution());
}